...
public class FormularioClientesController {
	...
	public ModelAndView insertarCliente(@Valid @ModelAttribute Cliente cliente, BindingResult br) {
		...
		Set<ConstraintViolation<Cliente>> cv = v.validate(cliente);		
		if(cv.size()>0) {
		...
		*/
		...
		//
		//POST-REDIRECT-GET
		ModelAndView mav = new ModelAndView("redirect:verListadoClientes"); //Esto no es el nombre de ninguna vista
		return mav;
	}	
	...
	public ModelAndView modificarCliente(@ModelAttribute Cliente cliente) {
		...
		ModelAndView mav = new ModelAndView("redirect:verListadoClientes");
		return mav;
	}
	
	@PostMapping("/borrarCliente")
	public String borrarCliente(@ModelAttribute Cliente cliente) {
		System.out.println("FormularioClientesController.borrarCliente");

		gestorClientes.borrar(cliente);
		
		return "redirect:verListadoClientes";
	}
